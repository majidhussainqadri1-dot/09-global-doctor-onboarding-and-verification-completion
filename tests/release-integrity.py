...
if package.get('versionInfo')!='1.2.0-RC3' or package.get('filesAnalyzed') is not True: fail('incomplete SBOM package identity')
for item in sbom.get('files',[]):
    ...
    if hashlib.sha256((root/rel).read_bytes()).hexdigest() not in values: fail('SBOM checksum mismatch: '+rel)
...
if sbom_paths != set(release_files)-{'SBOM.spdx.json'}: fail('SBOM coverage mismatch')
manifest=(root/'RELEASE-MANIFEST-1.2.0.md').read_text()
if 'global-doctor-onboarding-09/' not in manifest or 'global-doctor-onboarding-09-1.2.0-RC3.zip' not in manifest: fail('canonical package identity mismatch')
lock=json.loads((root/'RELEASE-LOCK.json').read_text())
if lock.get('runtime')!='1.2.0' or lock.get('schema')!=6 or lock.get('release_candidate')!='RC3' or lock.get('release_file_count')!=len(release_files): fail('release lock mismatch')
...
print('File 09 RC3 release-integrity checks passed.')
