...
for p in ['TRACEABILITY.md','RELEASE-MANIFEST-1.3.0.md','ADVANCED-TRUST-24.md','SBOM.spdx.json','REVIEW-ROUND-1.md','REVIEW-ROUND-2.md','REVIEW-40-ROUNDS-RC3.md','STAGING-ACCEPTANCE.md','MIGRATION-ROLLBACK-1.3.0.md','SECURITY-PRIVACY.md','OPERATIONS.md']:
...
if package.get('versionInfo')!='1.3.0-RC5' or package.get('filesAnalyzed') is not False: fail('SBOM repository template identity mismatch')
...
if 'global-doctor-onboarding-09/' not in manifest or 'global-doctor-onboarding-09-1.3.0-RC5.zip' not in manifest: fail('canonical package identity mismatch')
lock=json.loads((root/'RELEASE-LOCK.json').read_text(encoding='utf-8'))
if lock.get('runtime')!='1.3.0' or lock.get('schema')!=6 or lock.get('advanced_trust_schema')!=1 or lock.get('release_candidate')!='RC5' or lock.get('release_file_count')!=len(release_files): fail('release lock mismatch')
...
for token in ['generate_sbom',"release_candidate = 'RC5'", "version = '1.3.0'",'sabri:source-head','SBOM.spdx.json']:
    if token not in builder: fail('deterministic generated-SBOM builder missing '+token)
for token in ['generated SBOM','1.3.0-RC5','sabri:source-head','source/package parity mismatch']:
...
for p in ['includes/class-gdo-advanced-trust.php','includes/class-gdo-advanced-trust-events.php','ADVANCED-TRUST-24.md']:
    if p not in release_files: fail('advanced trust release file missing '+p)
for p in root.rglob('*'):
...
print('File 09 RC5 release-integrity checks passed.')
