...
for i in range(1,25):
    require(f'F09-AT-{i:02d}' in trace,'advanced trust trace missing '+str(i))
...
for token in ['RC5','advanced-trust-24.py','global-doctor-onboarding-09-1.3.0-RC5.zip']:
    require(token in workflow,'RC5 exact-head workflow missing '+token)
low=status.lower()
...
require(lock.get('release_candidate')=='RC5' and lock.get('runtime')=='1.3.0' and lock.get('advanced_trust_schema')==1 and lock.get('staging_accepted') is False and lock.get('live_deployed') is False and lock.get('operationally_accepted') is False,'release lock status falsehood')
print('File 09 latest central + File09 + Advanced Trust 24 parity gates passed.')
