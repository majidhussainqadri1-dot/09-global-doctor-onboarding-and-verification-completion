...
check(13, 'Reviewer object-scope/IDOR recheck', has(hard, 'reviewer_case_allows', "'gdo_check_forbidden'"))
...
check(18, 'Calibration excludes incomplete samples from outcome denominator', (has(trust, "'completed'!==sanitize_key($row['status'])", 'agreement_rate') or has(trust, "'completed' !== sanitize_key( $row['status'] )", 'agreement_rate')))
check(19, 'Existing quality-sampling ledger retained', has(quality, 'quality_samples', 'GDO_Quality', 'sample'))
...
check(37, 'Passport lifecycle revocation covers expired/suspended/revoked/rejected/withdrawn', all(x in hard for x in ["'expired'","'suspended'","'revoked'","'rejected'","'withdrawn'",'revoke_passports_for_application']))
...
check(56, 'Expired finalizing sessions are cleaned', has(hard, 'cleanup_upload_sessions', "'finalizing'", 'expires_at'))
check(57, 'Resumable upload terminates in canonical malware/type/encryption path with explicit private-temp provenance', has(trust, 'GDO_Evidence::stage_upload', 'stage_upload($app,$row->document_type,$file,true,$path)') and has(evidence, 'trusted_internal_path', 'native_uploaded && $filtered_uploaded'))
...
check(63, 'Advanced Trust REST check revalidates exact case relationship', has(hard, 'rest_check', 'reviewer_case_allows'))
...
check(70, 'Privacy export includes Advanced Trust records', 'privacy_export_rows' in privacy and 'GDO_Advanced_Trust::privacy_export_rows' in privacy and 'function privacy_export_rows' in trust)
check(71, 'Privacy erasure includes Advanced Trust and deletes passports collision-safely', has(privacy, 'GDO_Advanced_Trust_Hardening::privacy_erase_application') and 'verification_passports' in hard[hard.index('public static function privacy_erase_application'):hard.index('private static function rest_value')] and ('checked_delete' in hard[hard.index('public static function privacy_erase_application'):hard.index('private static function rest_value')] or '$wpdb->delete' in hard[hard.index('public static function privacy_erase_application'):hard.index('private static function rest_value')]))
