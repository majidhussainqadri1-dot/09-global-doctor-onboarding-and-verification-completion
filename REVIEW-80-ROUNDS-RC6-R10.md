...
## QA-harness corrections during R10

The first R10 evidence-synchronization run produced one **test-only false negative** at clean control R33. The static assertion looked for literal `SHOW COLUMNS` and the stale token `gdo_migration_lock` inside `class-gdo-migration.php`, although core physical postconditions are intentionally delegated to `GDO_Schema::verify_installation()` and the actual lock error is `gdo_migration_locked`. The assertion was corrected to verify the real semantic contract: migration lock option, lock-failure path and delegated installation verification. This did not uncover or change a product defect, so R33 remains a clean round and the product defect count remains 19.

A second **historical-harness-only coupling** appeared when the complete exact-head suite executed R9 under the newer R10 repository state. R9 controls 13 and 80 incorrectly required the *current* repository to remain permanently at R9's old 60-entry release count/current-status wording. R10 legitimately advances current release truth to 61 entries. The R9 test was therefore corrected to verify immutable R9 historical evidence — R9 ledger presence, frozen R9 baseline, R9 80/10/70 counts and continued R9 workflow execution — without forcing mutable current release metadata back to R9. This did not change File 09 product code or uncover a new R10 product defect, so the R10 defect-bearing set remains rounds 04–22 only.
