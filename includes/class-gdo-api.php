...
final class GDO_API {
    public static function latest_decision( $user_id ) {
        ...
        if ( ! $app ) {
            return array('state'=>'not_applied','verified'=>false);
        }
        ...
        $verified = GDO_State::public_verified($app->state) && ! $expired && ! GDO_Membership_Adapter::sanctioned($user_id) && ! empty($snapshot);
        return array(
            'application_id'   => absint($app->id),
            'application_uuid' => $app->application_uuid,
            'version'          => absint($app->version),
            'state'            => $expired ? 'expired' : $app->state,
            'verified'         => $verified,
            'verified_until'   => $app->verified_until,
            'fingerprint'      => $verified ? $app->approved_fingerprint : '',
        );
    }
...
function gdo_user_is_verified($user_id){$d=GDO_API::latest_decision($user_id);return !empty($d['verified']);}
function gdo_get_application_status($user_id){$d=GDO_API::latest_decision($user_id);return $d['state'];}
